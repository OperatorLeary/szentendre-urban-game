-- Phase 12: Player alias moderation hardening
-- Blocks numeric affix bypasses such as "fasz67" or "67fasz".
...
begin
  ...
  if exists (
    ...
    join unnest(blocked_tokens) as blocked(blocked_token)
      on alias_token.token ~ ('^[0-9]*' || blocked.blocked_token || '[0-9]*$')
  ) then
...
comment on function public.assert_player_alias_allowed(text) is
  'Raises an error when player alias contains blocked content or contact/link-style data. Also blocks numeric prefix/suffix bypasses around blocked tokens.';
